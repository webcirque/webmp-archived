...
function refresherThreadFunc() {
	...
	if (window.subt) {
		gui.time.innerHTML = " (" + (subt.currentLn + 1) + "/" + subt.sublist.length + ")";
	} else {
...
function Subtitles(text) {
	...
	this.caption = text;
	this.captionList = this.caption.split("\n\n");
	this.sublist = [];
	let count = 0;
	while (count < this.captionList.length) {
		if (this.captionList[count].trim() != "") {
			this.captionList[count] = this.captionList[count].split("\n");
			let timings = this.captionList[count][1].split(" --> ");
			let startArr = timings[0].split(":");
			let startT = 0;
			this.timeList = [];
			if (startArr[2].search(",")) {
				startArr[2] = startArr[2].replace(",", ".");
			}
			let don = 0;
			while (don < startArr.length) {
				startT += parseFloat(startArr[startArr.length - don - 1]) * Math.pow(60, don);
				don++;
			}
			startT = Math.round(startT * 100) / 100;
			this.timeList[count] = startT;
			let endArr = timings[1].split(":");
			let endT = 0;
			if (endArr[2].search(",")) {
				endArr[2] = endArr[2].replace(",", ".");
			}
			don = 0;
			while (don < endArr.length) {
				endT += parseFloat(endArr[endArr.length - don - 1]) * Math.pow(60, don);
				don++;
			}
			endT = Math.round(endT * 100) / 100;
			this.captionList[count].splice(0, 2);
			let donum = 0;
			let outsub = "";
			while (donum < this.captionList[count].length) {
				outsub += "<span>" + this.captionList[count][donum] + "</span><br/>";
				donum++;
			}
			this.sublist[count] = {
				"start": startT,
